...
const dragging = event => {
  ...
  let { thumbsize, bounds, padding } = switches.get(
    state.activethumb.parentElement,
  )
...
const preventBubbles = event => {
  if (state.recentlyDragged) event.preventDefault() && event.stopPropagation()
}
...
const determineChecked = () => {
  ...
  let curpos = Math.abs(
    parseInt(state.activethumb.style.getPropertyValue('--thumb-position')),
  )
